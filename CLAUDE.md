...
To always discard the identity on logout or expiration, set `$user->persistIdentity = false` (or the `persistIdentity` config option). Retaining the identity is best-effort and depends on the storage implementation.
...
An `IdentityHandler` may optionally implement `getGuestIdentity(): ?IIdentity` to provide an identity for anonymous (not logged-in) users. When present, `getIdentity()`, `getId()` and `getRoles()` fall back to it, so guests can carry their own roles and data instead of just the `$guestRole` string.
...
The guest identity is resolved on read only and is never written to storage. While not logged in, `getRoles()` (and thus `isAllowed()`/`isInRole()`) uses the guest roles.
...
security:
	authentication:
		...
		# Keep identity available after logout/expiration for personalization (defaults to true)
